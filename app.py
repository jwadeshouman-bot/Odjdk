...
def login():
    if request.method == "POST":
        ...
        if not username:
            return render_template_string(open("login.html").read(), error="Enter a username")
        data = load_data()
        user = data["users"].get(username)
        if user:
            stored_hash = user.get("password_hash", "")
            if stored_hash and stored_hash != hash_password(password):
                return render_template_string(open("login.html").read(), error="Wrong password")
        ...
        return redirect(url_for("dashboard"))
    return render_template_string(open("login.html").read(), error=None)
...
def dashboard():
    ...
    return render_template_string(open("dashboard.html").read(), servers=user_servers, running=running, total=len(user_servers), username=username)
...
def server_detail(name):
    ...
    return render_template_string(open("server.html").read(), server_name=name, config=cfg, files=files)
